...
public class Master extends ServantEquipment {
    ...
    private Map<String, CopyOnWriteArrayList<String>> domainWorkerMap = ServantGroup.getDomainWorkerMap();
    ...
    private IDistributionHandler distributionHandler = new IDistributionHandler() {
        ...
        public String distribution(Map<String, Object> source, CopyOnWriteArrayList<String> workerSet,
            ...
            if (source != null) {
                ...
                if (domain != null) {
                    if (domainWorkerMap.containsKey(domain) && domainWorkerMap.get(domain).size() > 0) {
                        int label = Math.abs(source.hashCode() % domainWorkerMap.get(domain).size());
    ...
    private IKeeperHandler handler = new IKeeperHandler() {
        ...
        public void takeLeadership(CuratorFramework client) throws Exception {
            ...
            try {
                ...
                ticketWindow = new MissionWindow(Master.this,ticketHost, ticketPort);
                workerCache.getListenable().addListener(new PathChildrenCacheListener() {
                    ...
                    public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
                        if (event.getType() == Type.CHILD_ADDED) {
                            ...
                            String workerName = path.substring(path.lastIndexOf("/") + 1);
                            String workDomain = new String(event.getData().getData(), Charset
                                    .forName(Constant.GLOBAL_CHARSET));
                            if (workDomain == null || workDomain.trim().equals("")
                                    || workDomain.toLowerCase().equals(DOMAIN_ALL)) {
                                workDomain = DOMAIN_ALL;
                            }
                            logger.info(workerName + " join in;domain:" + workDomain);
                            ...
                            if (domainWorkerMap.get(workDomain) == null) {
                                ...
                                domainWorkerMap.get(workDomain).add(workerName);
                            }
                        }
                        if (event.getType() == Type.CHILD_REMOVED) {
                            //remove a worker
                            String path = event.getData().getPath();
                            String domainName = new String(event.getData().getData(), Charset
                                    .forName(Constant.GLOBAL_CHARSET));
                            String workerName = path.substring(path.lastIndexOf("/") + 1);
                            workerSet.remove(workerName);
                            if (domainWorkerMap.get(domainName) != null) {
                                domainWorkerMap.get(domainName).remove(workerName);
                            }
...
//                        ticketWindow.updateWorkerMap(workerSet, domainWorkerMap);
                    }
                });
                workerCache.start();
                initIndex();
                ...
                logger.info("master--->"+Master.this.getId()+"   ->   init!");
            } catch (Exception e) {
    ...
    public Master(String name, String connectString, Settings settings, String host, int port) {
    ...
    public void tryBeKeeper() {
    	...
    	{
    		try {
				uid = MASTER_SYBOL+"/" + InetAddress.getLocalHost().getHostAddress() + "@" + ticketPort;
        ...
        leaderSelector = new LeaderSelector(getZookeeperClient(), uid, new LeaderSelectorListener() {
            @Override
            public void stateChanged(CuratorFramework client, ConnectionState newState) {
            ...
            public void takeLeadership(CuratorFramework client) throws Exception {
                ...
                poolExecutor.scheduleWithFixedDelay(new SeekWork(), 0, tickTime, TimeUnit.SECONDS);
                poolExecutor.scheduleWithFixedDelay(new IndexBuilderTimer(), 0, tickTime, TimeUnit.HOURS);
                dumpTimer.schedule(new IndexDumpTaskManager(getRepositoryClient()), findDumpDate(), 1000*60*60*24);
                try {
                    closeLatch.await();
    ...
    protected class SeekWork extends Thread {
        ...
        public void run() {
            try {
                ...
                SearchResponse response = QueryTools.searchIndexAndType(getRepositoryClient(), dailyIndex,
                ...
                for (SearchHit searchHit : hits) {
                    String type = distributionHandler.distribution(searchHit.getSource(), workerSet, domainWorkerMap);
                    Map<String, Object> source = searchHit.getSource();
                    source.put(Constant.WORK_ASSIGN_SYMBOL, "true");
                    QueryTools.moveIndexData(getRepositoryClient(), dailyIndex, Constant.WORK_NOT_ASSIGN_TYPE,
                            type, searchHit.getId(), source);
                }
            } catch (Exception e) {
            	logger.error(e.getMessage(),e);
            }
